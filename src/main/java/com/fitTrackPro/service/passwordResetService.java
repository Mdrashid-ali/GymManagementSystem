...
public class passwordResetService {
	...
	public String createResetToken(String email) throws SQLException {
		...
		String insertToken = "INSERT INTO password_reset_tokens (user_id, token, expires_at) VALUES (?, ?, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 30 MINUTE))";
	...
	public boolean isValidToken(String token) throws SQLException {
		if (token == null || token.isBlank())
			return false;
		String sql = "SELECT token_id FROM password_reset_tokens WHERE token = ? AND used_at IS NULL AND expires_at > CURRENT_TIMESTAMP";
		try (Connection connection = DBConnection.getConnection();
