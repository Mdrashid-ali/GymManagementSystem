...
public class trainerDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (trainer != null) request.setAttribute("workoutPlans", workouts.findByTrainerId(trainer.getTrainerId()));
            ...
            if ("/trainer/attendance".equals(path)) {
                request.setAttribute("members", members.findAll());
                request.setAttribute("attendanceList", attendance.findAll());
            ...
            request.getRequestDispatcher("/trainer/workouts".equals(path) ? "/WEB-INF/pages/workoutPlan.jsp" : "/WEB-INF/pages/trainerDashboard.jsp").forward(request, response);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            String target = "/trainer/attendance".equals(request.getServletPath()) ? "/trainer/attendance" : "/trainer/create-workout";
            response.sendRedirect(request.getContextPath() + target + "?error=" + url("Unable to complete action."));
        }
    }

    private void handleAttendancePost(HttpServletRequest request, HttpServletResponse response) throws IOException, SQLException {
        String action = request.getParameter("action");
        if ("checkout".equalsIgnoreCase(action)) {
            Integer attendanceId = parseInt(request.getParameter("attendanceId"));
            if (attendanceId == null || !attendance.checkOut(attendanceId)) {
                response.sendRedirect(request.getContextPath() + "/trainer/attendance?error=" + url("Unable to check out attendance record."));
                return;
            }
        ...
        if (memberId == null) {
            response.sendRedirect(request.getContextPath() + "/trainer/attendance?error=" + url("Select a member to check in."));
            return;
        }
        attendance.checkIn(memberId);
        response.sendRedirect(request.getContextPath() + "/trainer/attendance?success=" + url("Member checked in."));
    }

    private void handleCreateWorkout(HttpServletRequest request, HttpServletResponse response) throws IOException, SQLException {
        user currentUser = (user) request.getSession().getAttribute("currentUser");
        trainer trainer = trainers.findByUserId(currentUser.getUserId());
        Integer memberId = parseInt(request.getParameter("memberId"));
        ...
        if (trainer == null || memberId == null || validationUtil.isBlank(request.getParameter("planName")) || start == null || end == null || end.before(start)) {
            response.sendRedirect(request.getContextPath() + "/trainer/create-workout?error=" + url("Invalid plan details."));
            return;
        }

        workoutPlan plan = new workoutPlan(trainer.getTrainerId(), memberId, request.getParameter("planName"), start, end);
