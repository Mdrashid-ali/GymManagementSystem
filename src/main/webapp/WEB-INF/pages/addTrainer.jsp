...
<body>
    ...
    <div class="main-content"><h1>Add New Trainer</h1><% if (request.getParameter("error") != null) { %><div class="alert alert-danger"><%= request.getParameter("error") %></div><% } %><div class="card"><div class="card-header"><h3>Trainer Information</h3></div><div class="card-body"><form id="addTrainerForm" action="${pageContext.request.contextPath}/admin/add-trainer" method="post" novalidate><div id="trainerValidationMessage" class="alert alert-danger trainer-validation-error" role="alert"></div>
        <div class="row"><div class="col-6"><div class="form-group"><label for="firstName">First Name *</label><input type="text" id="firstName" name="firstName" class="form-control" data-required-message="Please enter the trainer first name."><div class="field-error" data-error-for="firstName"></div></div></div><div class="col-6"><div class="form-group"><label for="lastName">Last Name *</label><input type="text" id="lastName" name="lastName" class="form-control" data-required-message="Please enter the trainer last name."><div class="field-error" data-error-for="lastName"></div></div></div></div>
        <div class="row"><div class="col-6"><div class="form-group"><label for="email">Email Address *</label><input type="email" id="email" name="email" class="form-control" data-required-message="Please enter the trainer email address."><div class="field-error" data-error-for="email"></div></div></div><div class="col-6"><div class="form-group"><label for="phone">Phone Number</label><input type="tel" id="phone" name="phone" class="form-control"></div></div></div>
        <div class="row"><div class="col-6"><div class="form-group"><label for="password">Password *</label><div class="password-field"><input type="password" id="password" name="password" class="form-control" data-required-message="Please enter a password."><button type="button" class="password-eye" data-target="password" aria-label="Show password" title="Show password">
    ...
    <script>
        ...
        var trainerMessage = document.getElementById('trainerValidationMessage');
        var requiredTrainerFields = ['firstName', 'lastName', 'email', 'password'];
        ...
        addTrainerForm.addEventListener('submit', function(event) {
            ...
            requiredTrainerFields.forEach(function(fieldId) {
                ...
                if (input.value.trim() === '') {
                    hasError = true;
                    showTrainerFieldError(input, input.dataset.requiredMessage);
                    if (!firstInvalid) {
                        firstInvalid = input;
                    }
                }
            ...
            if (emailInput.value.trim() !== '' && !emailInput.checkValidity()) {
                hasError = true;
                showTrainerFieldError(emailInput, 'Please enter a valid email address.');
                if (!firstInvalid) {
                    firstInvalid = emailInput;
                }
            }
            var passwordInput = document.getElementById('password');
            if (passwordInput.value.trim() !== '' && !isStrongPassword(passwordInput.value)) {
                hasError = true;
                showTrainerFieldError(passwordInput, passwordRuleMessage);
                if (!firstInvalid) {
