...
public class trainerDashboardServlet extends HttpServlet {
	...
	private void handleCreateWorkout(HttpServletRequest request, HttpServletResponse response)
		...
		if (trainer == null || memberId == null || validationUtil.isBlank(request.getParameter("planName"))
				|| start == null || end == null || end.before(start)) {
			response.sendRedirect(
					request.getContextPath() + "/trainer/create-workout?error=" + url("Invalid plan details."));
			return;
		}

		workoutPlan plan = new workoutPlan(trainer.getTrainerId(), memberId, request.getParameter("planName"), start,
				end);
