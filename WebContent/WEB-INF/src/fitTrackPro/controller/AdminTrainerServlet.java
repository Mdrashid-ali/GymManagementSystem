    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if ("/admin/delete-trainer".equals(path)) {
                ...
                response.sendRedirect(request.getContextPath() + "/admin/trainers?" + (deleted ? "success=" + url("Trainer deleted.") : "error=" + url("Trainer not found.")));
                return;
            }
            ...
            if (!validationUtil.isValidEmail(email) || !validationUtil.isStrongPassword(password)) {
                response.sendRedirect(request.getContextPath() + "/admin/add-trainer?error=" + url("Enter a valid email and a strong password."));
                return;
            }
            ...
            if (validationUtil.isBlank(trainer.getFirstName()) || validationUtil.isBlank(trainer.getLastName())) {
                response.sendRedirect(request.getContextPath() + "/admin/add-trainer?error=" + url("First name and last name are required."));
                return;
            }
            ...
            if ("/admin/update-trainer".equals(request.getServletPath())) target = "/admin/trainers";
            response.sendRedirect(request.getContextPath() + target + "?error=" + url("Unable to complete trainer action: " + rootMessage(e)));
        }
    }
