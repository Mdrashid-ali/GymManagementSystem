...
<title><%=isAdmin ? "Update Member" : "My Profile"%> -
	FitTrack Pro</title>
<link rel="stylesheet"
...
<body>
	...
	<div class="sidebar">
		<ul class="sidebar-menu">
			...
			<li class="sidebar-item"><a
				...
				class="sidebar-link active">Manage Members</a></li>
			<li class="sidebar-item"><a
	...
	<div class="sidebar">
		<ul class="sidebar-menu">
			...
			<li class="sidebar-item"><a
				...
				class="sidebar-link active">My Profile</a></li>
			<li class="sidebar-item"><a
				href="${pageContext.request.contextPath}/member/attendance"
				class="sidebar-link">Attendance History</a></li>
			<li class="sidebar-item"><a
	...
	<div class="main-content profile-shell">
		...
		<div class="profile-hero">
			<div class="profile-avatar"><%=member.getFirstName() != null && !member.getFirstName().isBlank() ? member.getFirstName().substring(0, 1)
		: "M"%></div>
			<div class="profile-title">
				<h1><%=isAdmin ? "Edit Member" : "My Profile"%></h1>
				<p>
					<strong><%=member.getFullName()%></strong> Ãƒâ€šÃ‚Â·
					<%=member.getEmail()%></p>
			</div>
			<div class="profile-badges">
				<span
					class="badge <%=membershipActive ? "badge-success" : "badge-warning"%>"><%=membershipActive ? "Active Membership" : "Expired Membership"%></span>
		...
		<div class="profile-metric-grid">
			<div class="profile-metric">
				<span>Expiry Date</span> <strong><%=member.getMembershipExpiryDate() == null ? "Not set"
		: dateUtil.formatDisplayDate(member.getMembershipExpiryDate())%></strong>
			</div>
			<div class="profile-metric">
				<span>Fitness Goal</span> <strong><%=member.getFitnessGoal() == null || member.getFitnessGoal().isBlank() ? "Not set" : member.getFitnessGoal()%></strong>
			</div>
			<div class="profile-metric">
				<span>BMI</span> <strong><%=bmi == 0 ? "Not available" : String.format("%.1f", bmi) + " " + bmiStatus%></strong>
			</div>
			<div class="profile-metric">
				<span>Phone</span> <strong><%=member.getPhone() == null ? "Not set" : member.getPhone()%></strong>
			</div>
		</div>

		<form
			action="${pageContext.request.contextPath}/<%= isAdmin ? "admin/update-member" : "member/update" %>"
			method="post">
						...
						<div class="col-6">
							<div class="form-group">
								<label for="email">Email Address</label> <input type="email"
									...
									value="<%=member.getEmail()%>" readonly disabled> <small
			...
			<div class="card profile-form-card">
				...
				<div class="card-body">
						...
						<div class="col-6">
							<div class="form-group">
								<label for="membershipType">Membership Type</label> <select
									...
									<option value="BASIC"
										<%="BASIC".equals(member.getMembershipType()) ? "selected" : ""%>>Basic
										- NPR 4,500/month</option>
									<option value="PREMIUM"
										<%="PREMIUM".equals(member.getMembershipType()) ? "selected" : ""%>>Premium
										- NPR 7,500/month</option>
									<option value="FAMILY"
										<%="FAMILY".equals(member.getMembershipType()) ? "selected" : ""%>>Family
										- NPR 13,500/month</option>
									<option value="STUDENT"
										<%="STUDENT".equals(member.getMembershipType()) ? "selected" : ""%>>Student
										- NPR 3,000/month</option>
								</select>
							</div>
						</div>
						<div class="col-6">
							<div class="form-group">
								...
								<input type="date" id="membershipExpiryDate"
									name="membershipExpiryDate" class="form-control"
					...
					<div class="row">
						<div class="col-6">
							<div class="form-group">
								<label>Membership Type</label> <input type="text"
									class="form-control" value="<%=member.getMembershipType()%>"
									readonly disabled>
							</div>
						</div>
						<div class="col-6">
							<div class="form-group">
								<label>Membership Expiry Date</label> <input type="text"
									class="form-control"
									value="<%=member.getMembershipExpiryDate()%>" readonly
									disabled>
							</div>
						</div>
					</div>
				...
				<div class="card-body">
						...
						<div class="col-6">
							<div class="form-group">
								<label for="dateOfBirth">Date of Birth</label> <input
									...
									class="form-control" value="<%=member.getDateOfBirth()%>"
									<%=isAdmin ? "" : "readonly"%>>
							</div>
						</div>
						<div class="col-6">
							<div class="form-group">
								<label for="gender">Gender</label> <select id="gender"
									...
									<option value="MALE"
										<%="MALE".equals(member.getGender()) ? "selected" : ""%>>Male</option>
									<option value="FEMALE"
										<%="FEMALE".equals(member.getGender()) ? "selected" : ""%>>Female</option>
									<option value="OTHER"
										<%="OTHER".equals(member.getGender()) ? "selected" : ""%>>Other</option>
								</select>
							</div>
						</div>
					</div>
					<div class="form-group">
						...
						<textarea id="address" name="address" class="form-control"
							rows="2"><%=member.getAddress() != null ? member.getAddress() : ""%></textarea>
					</div>
				</div>
			</div>
			...
			<div class="card profile-form-card">
				...
				<div class="card-body">
					<div class="row">
						<div class="col-6">
							<div class="form-group">
								<label for="emergencyContactName">Contact Name</label> <input
									type="text" id="emergencyContactName"
						...
						<div class="col-6">
							<div class="form-group">
								<label for="emergencyContactPhone">Contact Phone</label> <input
									type="tel" id="emergencyContactPhone"
			...
			<div class="card profile-form-card">
				...
				<div class="card-body">
					<div class="row">
						<div class="col-3">
							<div class="form-group">
								<label for="height">Height (cm)</label> <input type="number"
									step="0.1" id="height" name="height" class="form-control"
						...
						<div class="col-3">
							<div class="form-group">
								<label for="weight">Weight (kg)</label> <input type="number"
									step="0.1" id="weight" name="weight" class="form-control"
						...
						<div class="col-3">
							<div class="form-group">
								<label for="bodyFat">Body Fat (%)</label> <input type="number"
									step="0.1" min="0" max="100" id="bodyFat" name="bodyFat"
									class="form-control" placeholder="e.g., 18.5">
							</div>
						</div>
						<div class="col-3">
							<div class="form-group">
								<label for="muscleMass">Muscle Mass (kg)</label> <input
									type="number" step="0.1" min="0" id="muscleMass"
									name="muscleMass" class="form-control" placeholder="e.g., 32.0">
							</div>
						</div>
						<%
						}
					...
					<div class="form-group">
						...
						<textarea id="fitnessGoal" name="fitnessGoal" class="form-control"
							rows="3"><%=member.getFitnessGoal() != null ? member.getFitnessGoal() : ""%></textarea>
					</div>
					<%
					if (!isAdmin) {
					%>
					<div class="form-group">
						...
						<textarea id="fitnessNotes" name="fitnessNotes"
							class="form-control" rows="2"
							placeholder="Progress notes, target changes, measurements, or trainer feedback"></textarea>
						<small class="text-muted">Saving this profile creates a
							new fitness history entry shown on your dashboard.</small>
					</div>
					<%
					}
					%>
					...
					<div class="form-group">
						...
						<textarea id="medicalNotes" name="medicalNotes"
							class="form-control" rows="3"><%=member.getMedicalNotes() != null ? member.getMedicalNotes() : ""%></textarea>
					</div>
					<%
					}
					%>
			...
			<div class="profile-actions">
				<div class="d-flex justify-content-between">
					<a
						href="<%=isAdmin ? request.getContextPath() + "/admin/members" : request.getContextPath() + "/memberDashboard"%>"
